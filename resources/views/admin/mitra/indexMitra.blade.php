            ...
            <div class="d-block mb-4 mb-md-0">
                ...
                <div class="mb-3">
                    <form action="{{ route('mitra.list') }}" method="GET">
                        <div class="row">
                            <div class="col-md-6">
                                <select name="jenisMitra" onchange="this.form.submit()" class="form-select">
                                    <option value="">All</option>
                                    <option value="Platinum" {{ request('jenisMitra') == 'Platinum' ? 'selected' : '' }}>
                                        Platinum</option>
                                    <option value="Gold" {{ request('jenisMitra') == 'Gold' ? 'selected' : '' }}>Gold
                                    </option>
                                    <option value="Silver" {{ request('jenisMitra') == 'Silver' ? 'selected' : '' }}>Silver
                                    </option>
                                </select>
                            </div>
                            <div class="col-md-6">
                    ...
                    <table class="table table-centered table-nowrap mb-0 rounded">
                        ...
                        <tbody>
                            @foreach ($dataMitra as $row)
                                <tr>
                                    <td>{{ ($dataMitra->currentPage() - 1) * $dataMitra->perPage() + $loop->iteration }}
                                    </td>
                                    ...
                                    <td>
                                        <span
                                            class="badge
                                    {{ $row->jenisMitra == 'Platinum'
                                        ? 'bg-info'
                                        : ($row->jenisMitra == 'Gold'
                                            ? 'bg-warning'
                                            : ($row->jenisMitra == 'Silver'
                                                ? 'bg-secondary'
                                                : '')) }}">
                                            {{ $row->jenisMitra }}
                                        </span>
